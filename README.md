...
**此版本为个人翻译版本，翻译的初衷是为了帮助国内研究者更低成本的阅读和学习，不得用于商业行为！**

原项目连接：https://github.com/google-research/tuning_playbook

## 目录

-   [谁需要阅读该文档?](#谁需要阅读该文档?)
-   [为什么编写调优手册?](#为什么编写调优手册?)
-   [开始一个新项目的指南](#开始一个新项目的指南)
    -   [模型架构选择](#模型架构选择)
    -   [优化器选择](#优化器选择)
    -   [选择batch size](#选择batch size)
    -   [选择初始配置](#选择初始配置)
-   [能提高模型性能的科学方法](#能提高模型性能的科学方法)
    -   [增量的调优策略](#增量的调优策略)
    -   [探索与利用](#探索与利用)
    -   [选择下一轮实验的目标](#选择下一轮实验的目标)
    -   [设计下一轮实验](#设计下一轮实验)
    -   [决定是否采用一个训练流程的更改或者超参数配置](#决定是否采用一个训练流程的更改或者超参数配置)
    -   [研究结束后](#研究结束后)
-   [确定每次训练的步数](#确定每次训练的步数)
    -   [当训练不受计算限制时，决定训练时长](#D当训练不受计算限制时，决定训练时长)
    -   [当训练计算受限时，决定训练时长](#当训练计算受限时，决定训练时长)
-   [训练流程的额外指导](#训练流程的额外指导)
    -   [优化输入流程](#优化输入流程)
    -   [评估模型性能](评估模型性能)
    -   [保存检查点并回顾性的选择最优检查点](#保存检查点并回顾性的选择最优检查点)
    -   [设置实验跟踪](#设置实验跟踪)
    -   [Batch normalization的实现细节](#Batch normalization的实现细节)
    -   [多主机管道的考虑因素](#多主机管道的考虑因素)
-   [常见问题](#常见问题)
-   [致谢](#致谢)
-   [引用](#引用)
-   [贡献](#贡献)
...
本文档面向工程师和研究人员（个人和团队）以及所有对最大化深度学习模型的性能感兴趣的人。

我们会假设您已经拥有机器学习和深度学习概念的基础知识。

我们的重点是超参数调优的过程。我们还会提到深度学习训练的其他方面，比如流程的实现和优化，但我们对这些方面并不打算完整的说明。

我们假设机器学习问题是一个有监督的学习问题或者其他看起来很像的问题（例如，自监督）。也就是说，这份文档中的方法也可能适用于其他类型的问题。
...
目前，为了让深度神经网络在实践中很好的工作，涉及到惊人数量的辛劳和猜想工作。更糟糕的是，人们很少记录让深度学习获得良好效果的方法。论文掩盖了得出最终结果的过程以呈现一个更干净的故事，机器学习工程师正在为商业问题努力，而很少有时间归纳总结他们解决问提的方法和过程。教科书往往回避实用指导，优先考虑基础知识原则，即使作者在应用的工作中有必要的经验可以提供有用的建议。在准备创建本文档时，我们找不到任何全面的尝试来真正解释“如何使用深度学习获得良好的结果”。相反，我们在博客文章和社交媒体上找到了一些建议的片段，在研究论文的附录中发现了一些技巧，偶尔会有关于某个特定项目或管道的案例研究，还有很多困惑。深度学习专家和技术水平较低的从业者使用表面上相似的方法所取得的结果之间存在着巨大的鸿沟。与此同时，这些专家欣然承认，他们所做的一些事情可能并不完全合理。随着深度学习的成熟并对世界产生更大的影响，社区需要更多的资源来涵盖有用的方法，包括所有对获得良好结果而至关重要的实际细节。
...
我们是一个由五名研究人员和工程师组成的团队，他们在深度学习领域工作了多年，其中一些人早在2006年就开始了。我们已经将深度学习应用于从语音识别到天文学的所有问题，并在此过程中学到了很多东西。这份文档源于我们自己训练神经网络的经验，希望可以为新的机器学习工程师提供帮助，并就深度学习的实践为我们的同事提供建议。尽管看到深度学习从少数学术实验室实践的机器学习方法发展为数十亿人使用的产品技术是令人欣慰的，但深度学习作为一门工程学科仍处于起步阶段，我们希望这份文件可以鼓励其他人帮助系统化该领域的实验协议。
...
本文是在我们试图具体化自己的深度学习方法时提出的，因此它代表了作者在撰写本文时的观点，而不是任何客观事实。我们自己在超参数调优方面的努力使其成为我们指导的一个特别重点，但我们也讨论了在工作中遇到的其他重要问题(或看到出错)。我们的目的是让这项工作成为一个活的文件，随着我们信念的改变而成长和发展。例如，关于debugging和减轻培训失败的材料在两年前是不可能写出来的，因为这份文档是基于最近的结果和正在进行的调查编写的。不可避免的是，我们的一些建议需要更新，以考虑到新的结果和改进的工作流程。我们不知道最佳的深度学习方法，当社区开始记录和辩论不同的方法时，我们才有希望找到它。为此，我们鼓励发现我们建议中问题的读者提出替代建议，以及令人信服的证据，这样我们就可以更新剧本。我们也很乐意看到可能有不同建议的替代指南和剧本，这样我们就可以作为一个社区努力实现最佳实践。最后，任何标有🤖表情符号的区域都是我们想要做更多研究的地方。只有在尝试编写这本剧本之后，才完全清楚在深度学习从业者的工作流程中可以找到许多有趣的和被忽视的研究问题。
...
我们在调优过程中做出的许多决策可以在项目开始时一次性做出，只有在环境发生变化时才会偶尔重新审视。
...
* 在问题表述、数据清洗等方面已经完成了足够多的基本工作，因此花时间在模型架构和训练配置上是有意义的。
* 已经设置了一个流程来进行训练和评估，并且很容易对各种感兴趣的模型执行训练和预测工作。
* 已经选择并实现了适当的评估方法。这些应该尽可能具有在部署环境中评估的代表性。
...
***总结：**当开始一个新工作的时候，优先尝试重用一个已经存在的模型。*
...
-   Choose a well established, commonly used model architecture to get working
    ...
    首先选择一个优秀、常用的模型架构来开始工作，随后在进行一些自定义模型的工作。
...
-   Model architectures typically have various hyperparameters that determine
    ...
    模型架构通常由各种超参数决定模型的大小和其他细节（例如：层数，层宽，各种类型的激活函数）
    ...
    -   Thus, choosing the architecture really means choosing a family of
        ...
        因此，选择架构实际上意味着选择了一个家族的不同模型（配置不同的模型超参数）
        
    -   We will consider the problem of choosing the model hyperparameters in
        [Choosing the initial configuration](#choosing-the-initial-configuration) and [A scientific approach to improving model performance](#a-scientific-approach-to-improving-model-performance).
        
        我们将如何选择模型超参数的问题放在了以下两个章节：[选择初始配置](#选择初始配置)和[能提高模型性能的科学方法](#能提高模型性能的科学方法)
...
-   When possible, try to find a paper that tackles something as close as
    ...
    如果可以，尝试找到一篇论文，找到一些可能可以解决手头问题的方法，并复现该模型来作为起点。
...
-   No optimizer is the "best" across all types of machine learning problems and
    ...
    在所有类型的机器学习问题和模型架构中，没有一个优化器是“最好的”，即使只是[comparing the performance of optimizers is a difficult task](https://arxiv.org/abs/1910.05446).
    🤖
    
-   We recommend sticking with well-established, popular optimizers, especially
    when starting a new project.
    
    我们尤其建议坚持使用成熟、流行的优化器来开始一个新的项目。
    
    -   Ideally, choose the most popular optimizer used for the same type of
        problem.
        
        理想情况下，可以选择最流行的优化器用于同一类型的问题。
...
-   Be prepared to give attention to ***all*** hyperparameters of the
    ...
    -   Optimizers with more hyperparameters may require more tuning effort to
        ...
        具有更多超参数的优化器可能 需要更多的调整工作以找到最佳配置。
    ...
    -   This is particularly relevant in the beginning stages of a project when
        ...
        在项目的初始阶段，当我们试图找到各种其他超参数(例如，架构超参数)的最佳值时，这尤其重要，同时我们将优化器超参数视为[nuisance parameters](#identifying-scientific-nuisance-and-fixed-hyperparameters).
    ...
    -   It may be preferable to start with a simpler optimizer (e.g. SGD with
        ...
        项目初期最好从一个更简单的优化器开始（例如：固定动量的SGD、固定$\epsilon$, $\beta_{1}$, 和
        $\beta_{2}$的Adam)，之后可以更换更多的普遍被使用的优化器。
...
-   The batch size should *not be* treated as a tunable hyperparameter for
    ...
    -   As long as all hyperparameters are well-tuned (especially the learning
        ...
        只要所有超参数都已经调整好（特别是学习率和正则化超参数）和训练的数量步骤足够，应该可以达到相同的最终性能，使用任意批处理大小（参见[Shallue et al. 2018](https://arxiv.org/abs/1811.03600)）
...
-   For a given model and optimizer, there will typically be a range of batch
    ...
    对于给定的模型和优化器，可用的硬件通常支持一系列的批处理大小。限制因素通常是加速器内存。
...
-   Unfortunately, it can be difficult to calculate which batch sizes will fit
    ...
    不幸的是，我们可能很难计算多大的batch size在内存中会导致运行失败，或者至少完成编译或者完整的训练程序。
...
-   The easiest solution is usually to run training jobs at different batch
    ...
    最简单的解决方案通常是在不同批次运行训练任务，每次增加一点点batch size（例如增加2的幂次），知道其中一次训练任务超过可用内存。
    
-   For each batch size, we should train for long enough to get a reliable
    estimate of the *training throughput*
    
    对于每个batch size，我们应该训练足够长的时间来获得一个可靠的训练吞吐量的估计：
...
<p align="center">训练吞吐量=每秒处理的例子数，或者，相当于每一步的时间</p>

<p align="center">每一步的时间=(批量大小)/(训练吞吐量)</p>
...
- When the accelerators aren't yet saturated, if the batch size doubles, the
  ...
  当加速器尚未饱和时，如果batch size翻倍，则训练吞吐量也应该翻倍（或者接近翻倍）。同样，每一步的时间也应该是恒定的（或者至少接近常数）随着batch size的增加。
...
- If this is not the case then the training pipeline has a bottleneck such as
  ...
  如果不是这样，那么训练流水线就会出现瓶颈，例如：I/O或计算节点间的异步通讯，这可能值得在继续训练之前进行诊断并纠正。
...
- If the training throughput increases only up to some maximum batch size,
  ...
  如果训练吞吐量只增加到某个最大batch size，那么我们应该只考虑达到该最大batch size的batch size，即使硬件支持更大的batch size。
  ...
  -   All benefits of using a larger batch size assume the training throughput
      ...
      使用更大的batch size的所有好处都是假设训练吞吐量增加。如果没有增加，则应当解决瓶颈问题或使用较小的batch size。
  ...
  -   **Gradient accumulation** simulates a larger batch size than the
      ...
      **梯度累积**模拟的batch size大于硬件所能支持的大小，因此不能提供任何吞吐量优势。在实际工作中一般应避免使用。
...
- These steps may need to be repeated every time the model or optimizer is
  ...
  每次使用模型或优化器时，可能需要重复这些步骤(例如，不同的模型架构可能允许更大的batch size
...
<p align="center">训练时间=每一步的时间×总步数</p>
...
-   We can often consider the time per step to be approximately constant for all
    ...
    我们通常可以认为每一步的时间对所有的batch size都是恒定的。如果并行计算没有带来额外开销，并且所有的训练瓶颈都已经被诊断和纠正，那么这是正确的。（参见先前的[章节](#确定可行的batch size并估计训练吞吐量)，怎样确定训练的瓶颈）在实践中，通常增加batch size至少会带来一点开销。
...
-   Therefore, the batch size that minimizes training time is usually the
    ...
    因此，最小化训练时间的batch size通常是最大的batch size，但仍然可以减少所需的训练步骤数量。
    ...
    -   This batch size depends on the dataset, model, and optimizer, and it is
        ...
        batch size取决于数据集、模型和优化器，如何计算它，而不是通过实验为每个新问题找到它，是一个开放问题。🤖
    ...
    -   When comparing batch sizes, beware the distinction between an example
        ...
        在比较batch size时，请注意示例预算/epoch预算(运行所有实验，同时固定训练示例的展示数量)和步骤预算(运行所有实验，训练步骤的数量固定)之间的区别。（不知道怎么翻译，没搞明白这句话的意思）
        ...
        -   Comparing batch sizes with an epoch budget only probes the perfect
            ...
            将batch size与epoch预算进行比较只能探索完美的扩展机制，即使更大的batch size仍然可以通过减少所需的训练步骤数量来提供有意义的加速。
    ...
    -   Often, the largest batch size supported by the available hardware will
        ...
        通常，可用硬件支持的最大批处理大小小于临界批处理大小。因此，一个好的经验法则(不需要运行任何实验)是使用尽可能大的批处理大小。
    
-   There is no point in using a larger batch size if it ends up increasing the
    training time.
    
    如果使用更大的batch size最终会增加训练时间，那么就没有意义了。

</details>

#### 选择batch size以最小化资源消耗
...
-   If there are significant upfront costs to increasing the batch size, it
    ...
    如果怎更加batch size有显著的前期成本，那么推迟增加batch size的计划可能更好，知道项目成熟，更容易评估成本与效益的权衡。实现多主机并行训练程序可能会引入[bug](#considerations-for-multi-host-pipelines) 和[subtle issues](#batch-normalization-implementation-details)，因此从更简单的流程可能更好。（另一方面，在需要大量调优实验的过程早期，训练时间的大幅加快可能是非常有益的）
...
​		我们把总使用成本(可能包括多种不同的成本)称为“资源消耗”。我们可以将资源消耗分解为以下几个部分:

<p align="center">资源消耗 = 每一步的资源消耗 x 总步数</p
...
- Increasing the batch size usually allows us to
  ...
  增加batch size通常可以让我们减少总的训练步数。资源消耗是增加还是减少，要看每一步的消耗变化。
  ...
  -   Increasing the batch size might *decrease* the resource consumption. For
      ...
      增加batch size可能会减少资源消耗。例如batch size较大的每个步骤都可以与batch size较小的每个步骤运行在相同的硬件上(每个步骤只增加少量的时间)，那么每个步骤的资源消耗的增加可能会被步骤数的减少所抵消。
  ...
  -   Increasing the batch size might *not change* the resource consumption.
      ...
      增加batch size可能不会改变资源消耗。例如，如果batch size加倍，所需的步骤数减半，使用的gpu数量也加倍，那么总消耗(以gpu小时计)不会改变。
  ...
  -   Increasing the batch size might *increase* the resource consumption. For
      ...
      增加batch size可能会增加资源消耗。例如，如果增加batch size需要升级硬件，那么每一步消耗的增加可能超过步骤数的减少。
...
-   The optimal values of most hyperparameters are sensitive to the batch size.
    ...
    大多数超参数的最优值对batch size敏感。因此，改变批量大小通常需要再来一遍调优。
    
- The hyperparameters that interact most strongly with the batch size, and therefore are most important to tune separately for each batch size, are the optimizer hyperparameters (e.g. learning rate, momentum) and the regularization hyperparameters.

  与batch size交互最强烈的超参数是优化器超参数(例如学习率，动量)和正则化超参数，因此对于每个不同大小的batch size单独调整从好参数是最重要的。
...
-   Keep this in mind when choosing the batch size at the start of a project. If
    ...
    在项目开始时选择batch size时，请记住这一点。如果您稍后需要切换到不同的batch size，那么为新的批量大小重新调整所有内容可能是困难的、耗时的和昂贵的。

</details>

#### batch norm与batch size如何互相作用
...
-   Batch norm is complicated and, in general, should use a different batch size
    ...
    batch norm比较复杂，一般来说，在计算统计量时，应该使用与梯度计算不同的batch size。详情请参阅[batch norm section](#batch-normalization-implementation-details)。
...
-   Our guiding principle is to find a simple, relatively fast, relatively
    ...
    -   "Simple" means avoiding bells and whistles wherever possible; these can
        ...
        “简单”意味着尽可能避免花哨的东西;这些总是可以稍后添加。即使那些花里胡哨的东西在将来被证明是有用的，将它们添加到初始配置中可能会浪费时间调整无用的功能，并导致不必要的复杂化。
        
        -   For example, start with a constant learning rate before adding fancy
            decay schedules.
            
            例如，在添加花哨的东西之前，先从一个恒定的学习率开始设置衰减器。
    ...
    -   Choosing an initial configuration that is fast and consumes minimal
        ...
        选择一个快速且消耗最少的初始配置，少量的资源将使超参数调优更加高效。
    ...
    -   "Reasonable" performance depends on the problem, but at minimum means
        ...
        “合理”的性能取决于问题，但至少意味着训练好的模型在验证集上的表现要比随机的好得多(尽管它可能糟糕到不值得部署)。
...
-   选择训练步骤的数量涉及以下方面的平衡关系:

    * 一方面，训练更多的步骤可以提高性能，并使超参数调优更容易(参见[Shallue et al. 2018](https://arxiv.org/abs/1811.03600)).

    * 另一方面，更少的步骤训练意味着每次训练运行更快，使用更少的资源，通过减少周期之间的时间来提高调优效率，并允许更多的实验并行运行。此外，如果一开始选择了不必要的大步长策略，以后可能很难更改它，例如，一旦针对该步数调整了学习率调度。
...
就本文而言，机器学习开发的最终目标是最大化已部署模型的效用。尽管不同应用程序的开发过程在许多方面不同(例如时间长度、可用的计算资源、模型类型)，但我们通常可以在任何问题上使用相同的基本步骤和原则。
...
* 有足够的计算资源可以进行有意义的调优实验，并至少并行运行几个训练任务。
...
***总结：***从简单的配置开始，在深入了解问题的同时逐步进行改进。确保任何改进都是基于强有力的证据，以避免增加不必要的复杂性

-   Our ultimate goal is to find a configuration that maximizes the performance
    of our model.
    
    我们的最终目标是找到一种能将我们的模型性能最大化的配置。
...
-   In principle, we could maximize performance by using an algorithm to
    ...
    原则上，我们可以通过使用一种算法来自动搜索整个可能配置的空间来最大化性能，但这不是一个实际的选择。
    ...
    -   The space of possible configurations is extremely large and there are
        ...
        可能的配置空间是非常大的，目前还没有任何复杂的算法可以在没有人类指导的情况下有效地搜索这个空间。
...
-   Most automated search algorithms rely on a hand-designed *search space* that
    ...
    大多数自动化搜索算法依赖于手工设计的搜索空间定义要搜索的配置集，这些搜索空间很重要。
...
-   The most effective way to maximize performance is to start with a simple
    ...
    最大化性能的最有效方法是从一个简单的开始配置，之后增量添加功能并进行改进，在这个过程中建立对问题的洞察力。
    ...
    -   我们在每一轮调优中使用自动搜索算法，并随着我们的理解不断更新搜索空间。
